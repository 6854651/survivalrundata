...
def update_db():
    ...
    try:
        with st.spinner("Updating database… This may take a few moments."):
            # Call the update script synchronously
            subprocess.run(["python", UPDATE_SCRIPT], check=True)
            # If temp_db exists, replace DB_FILE atomically
...
if needs_update(DB_FILE):
    update_db()  # runs synchronously with spinner
...
if mode == "Compare Athletes":
    ...
    with col1:
        typed_name = st.text_input("Search athlete (type first or last name)", placeholder="Start typing a name...")
        suggestions = get_name_suggestions(typed_name, years_compare) if typed_name else []
        selected_to_add = st.selectbox("Matching athletes", options=[""] + suggestions, label_visibility="collapsed")
    ...
    if st.session_state.selected_athletes:
        ...
        for i, athlete in enumerate(st.session_state.selected_athletes):
            with cols[i]:
                st.button(f"❌ {athlete}", key=f"remove_{athlete}", on_click=lambda a=athlete: st.session_state.selected_athletes.remove(a))
...
# ============================
    ...
    athlete_name = st.selectbox("Click to select athlete", options=[""] + suggestions)
